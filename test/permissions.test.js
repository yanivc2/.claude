...
test('a "manager" (secretary + granted perms) can manage suppliers; a plain secretary cannot', async () => {
  ...
  const manager = await createUser(
    { name: 'מנהל', username: 'mgr', role: 'secretary', label: 'מנהל', permissions: ['manage_suppliers'], password: 'Pass1234' },
    ow, db,
  );
  ...
  const plain = await createUser(
    { name: 'מזכירה', username: 'sec2', role: 'secretary', password: 'Pass1234' },
    ow, db,
  );
...
test('owner role ignores per-key permissions (always full)', async () => {
  ...
  const u = await createUser(
    { name: 'x', username: 'ux', role: 'owner', permissions: ['settings'], password: 'Pass1234' },
    ow, db,
  );
