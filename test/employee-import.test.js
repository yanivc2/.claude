...
test('lastReconciliationFor is per store (match via payment, auto via bank_account) and picks latest for all', async () => {
  const db = await freshDb();
  const ow = await owner(db);
  ...
  await db.run(
    `INSERT INTO audit_log (user_id, action, entity_type, entity_id, timestamp, details)
     VALUES (?, 'reconcile.match', 'payment', ?, '2026-08-01 10:00:00', '{}')`, [ow.id, pA.lastInsertRowid]);
  await db.run(
    `INSERT INTO audit_log (user_id, action, entity_type, entity_id, timestamp, details)
     VALUES (?, 'reconcile.auto', 'bank_account', ?, '2026-08-20 09:00:00', '{}')`, [ow.id, b.acct]);
  ...
  assert.equal(String(forA.ts).slice(0, 10), '2026-08-01');
