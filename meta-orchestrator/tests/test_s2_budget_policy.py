...
def test_frozen_paid_spend_ledger_loads_and_totals():
    ...
    # $0.052343 diagnostic + $0.064999 official ($0.023641 historical v1+v2.1 + final-sequence
    # black-112 $0.012028 + black-130 $0.029330, both SOLVED + lesson banked)
    assert led.total_paid_to_date() == Decimal("0.117342")
    assert Decimal(led.diagnostic_apparatus_spend_usd) == Decimal("0.052343")
    assert Decimal(led.official_training_spend_usd) == Decimal("0.064999")
...
def test_lifetime_spend_plus_worst_binds_global_cap():
    ...
    assert lifetime == Decimal("46.114092")
