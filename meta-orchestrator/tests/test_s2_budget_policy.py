...
def test_frozen_paid_spend_ledger_loads_and_totals():
    ...
    # $0.274750 official ($0.023641 historical + final-sequence black-112 $0.012028 + black-130
    ...
    # $0.035082 SOLVED+banked (DEFECT-6-fixed) + black-224 $0.036710 SOLVED+banked). The prior
    # black-215 DEFECT_6 run stays diagnostic-only.
    assert led.total_paid_to_date() == Decimal("0.396551")
    assert Decimal(led.diagnostic_apparatus_spend_usd) == Decimal("0.121801")
    assert Decimal(led.official_training_spend_usd) == Decimal("0.274750")
...
def test_lifetime_spend_plus_worst_binds_global_cap():
    ...
    assert lifetime == Decimal("46.393301")
