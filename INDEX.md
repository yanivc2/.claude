...
**🔴 העיקרון: תוכנה לא יכולה לעצור מישהו שנכנס לבנק.** הבנק לא שואל אותנו. לכן זה **לא נבנה כמנעול** אלא כך שהעברה לא-מתועדת **בלתי אפשרית להסתרה** — וזו הגרסה הברת-השגה של אותה מטרה.
...
| **אישור הבעלים — לפני שהכסף זז** | `POST /transfers/:id/approve` (**`requireOwner`**) | `approveTransfer` | הפעולה האנושית **היחידה** בזרימה. החשבונית במרחק לחיצה בשורה עצמה — **זה מה שמחליף את הוואטסאפ**. ביצוע לפני אישור נדחה במפורש. | הסרת `requireOwner` הופכת את כל הדף לחסר משמעות. |
