...
| 6 | **`footer.ejs` — enhancers גלובליים** | `partials/footer.ejs` | בורר-תאריך מחליף כל `input[type=date]`; קומבובוקס מחליף `<select class="js-combo">`. שינוי במבנה השדות שם משפיע על **כל** הטפסים. |
...
| תפריט **אפשרויות** (גם אחרי תשלום) | לינקים | `can('edit_invoice')` / `can('approve_payment')` | ✏️ ערוך חשבונית · 💳 ערוך אמצעי תשלום (אם יש תשלום) · ➕ לאותו ספק (`?supplier=&store=`). | הצגה גם ל-`status==='paid'` (בכוונה). ההרשאות מפרידות בין הפריטים. |
