...
| 6 | **`footer.ejs` — enhancers גלובליים** | `partials/footer.ejs` | בורר-תאריך מחליף כל `input[type=date]` (חשוף כ-`window.apEnhanceDates(root)` לשורות שנוספות דינמית, כמו הוצאות סגירת-Z); קומבובוקס מחליף `<select class="js-combo">`; **מקפל רובריקות** — כל `.card` עם `<h2>` ישיר הופך למתקפל (אקורדיון, נשמר ב-localStorage); **כלי-טבלה** — כל `<table>` עם `<thead>` ו-≥2 שורות מקבל חיפוש חי (מסנן כל העמודות בכל הקשה) + **כפתור "ייצוא" יחיד** (`.tbl-export-toggle`) שפותח תפריט (`.tbl-export-menu`, מוסתר עם `[hidden]`) עם Excel/CSV/PDF של השורות הנראות (עמודות `.no-print`/`[data-noexport]` **מוחרגות** מהייצוא). דילוגים: `.no-collapse`/`.no-enhance`, `<details>`, כרטיס עם `[required]`. גם **`window.apAutoSubmit(el)`** — עוזר גלובלי לשליחת טופס מ-`onchange` (בורר חנות פעילה בבאנר, מסנני מרקורים/דוחות/התאמה): משתמש ב-`requestSubmit()` (ולא `form.submit()`) כי בתוך PWA מותקן ב-iOS שליחה תוכנתית עם `form.submit()` "בורחת" ל-Safari ("address is invalid"). כל בורר עם `onchange="apAutoSubmit(this)"` תלוי בו. שינוי שם משפיע על **כל** הדפים. |
