...
| 6 | **`footer.ejs` — enhancers גלובליים** | `partials/footer.ejs` | בורר-תאריך מחליף כל `input[type=date]`; קומבובוקס מחליף `<select class="js-combo">`; **מקפל רובריקות** — כל `.card` עם `<h2>` ישיר הופך למתקפל (אקורדיון, נשמר ב-localStorage); **כלי-טבלה** — כל `<table>` עם `<thead>` ו-≥2 שורות מקבל חיפוש חי (מסנן כל העמודות בכל הקשה) + ייצוא Excel/CSV של השורות הנראות (עמודות `.no-print`/`[data-noexport]` **מוחרגות** מהייצוא). דילוגים: `.no-collapse`/`.no-enhance`, `<details>`, כרטיס עם `[required]`. שינוי שם משפיע על **כל** הדפים. |
...
| קוביות (ספקים ממתינים, מוחזקות, צ׳קים בחוץ, מזומן לא-משויך, הפקדות, פערי Z) | `GET /` | `dashboardStats(scope, storeId)`, `outstandingChecks(scope, {storeId})`, `unmatchedCashExpenses`, `listDeposits`, `zSequenceStatus` | קריאה-בלבד; לינקים לדפי המקור. **כל הקוביות מכובדות להקשר החנות הפעילה** (`storeId`=activeStoreId): `dashboardStats` מסנן חשבוניות לפי `store_id`, `outstandingChecks` מסנן ב-JS את שורות-החשבון, וקוביית "צ׳קים בחוץ" (oc_store) מברירת-מחדל לחנות הפעילה. "תשלום במזומן ללא התאמה" מ**אחד** את `z_expenses`+`z_closing_expenses` (UNION, שדה `source`); כותרת הקובייה מציגה תג "N לטיפול" כשיש. | `dashboardStats` קיבל פרמטר מיקום `storeId` (בין scope ל-x) — קוראים חייבים `(scope, storeId, x)`. `outstandingChecks` storeId מסונן ב-JS (pg-mem לא תומך WHERE על עמודת JOIN ב-GROUP BY). |
