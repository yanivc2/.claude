...
| **דוח פדיון מידנייט** (רובריקה בדף רווחיות) | `POST /reports/profitability/revenue-import` (multer, ≤5MB) · קליטה אוטומטית: `POST /ingest/revenue-report?store=<id>&secret=…` | `lib/revenueReportFile.js` (parse) → `services/revenueReports.js` (`importRevenueRows`/`revenueInRange`/`listRevenue`), טבלת **`revenue_reports`** (סכימה ×3+migrate) | הדוח הלילי (סך מכירות + סליקות אשראי) פר חנות פר יום. **מקור המכירות השיטתי** — דוחות Z נרשמים בשעות לא קבועות ולא כל יום. `parseRevenueReport` מזהה כותרות עברית אוטומטית (תאריך/פדיון/אשראי) ותומך ב**מיפוי עמודות ידני** כשהזיהוי נכשל; תאריכים יום-ראשון + Excel serial; שורות כותרת מעל הכותרת האמיתית מדולגות; יום שחוזר מסוכם. ייבוא **מחליף** יום קיים (upsert על `store_id+report_date`) — שליחה חוזרת של אותו מייל לא מכפילה. | **בדף רווחיות**: עמודות "דוח פדיון" + "מזה אשראי" מסוכמות לפי טווח התאריכים, וכשיש ימים בטווח ה**רווח מחושב מהן** (`usesRevenue`); חנות בלי דוח נופלת חזרה ל-Z (`zSales`) — אין רגרסיה. הקליטה מהמייל דורשת **`REVENUE_INGEST_SECRET`** (בלעדיו 503) ומותקנת **לפני** שער ההזדהות ב-`app.js`. |
