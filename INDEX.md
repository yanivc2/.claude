...
| **🔴 פרטי בנק של ספק (יעד ההעברה)** | `POST /suppliers/:id/bank` (**`requireOwner`**) | `services/suppliers.js#setSupplierBank` → `suppliers.bank_*` + **`supplier_bank_changes`** | **ההעברה המסוכנת אינה חשבונית מזויפת — היא חשבונית אמיתית, בסכום נכון, שאושרה כראוי, ששולמה לחשבון שהוחלף בשקט.** מייל "שינינו חשבון" הוא הדרך שבה עסק בגודל הזה מאבד כסף, ושום דבר בחשבונית לא נראה חריג. לכן: היעד יושב על הספק, שינוי הוא **פעולת בעלים**, כל שינוי נשמר עם מי ומתי + **פוש**, ושורת ההעברה מציגה את היעד ואת האזהרה "פרטי הבנק שונו לאחרונה" (`BANK_CHANGE_WARN_DAYS` = 60) **ליד כפתור האישור** — עם `confirm` שמזכיר לאמת טלפונית. `bankMissing` מסמן ספק בלי יעד. | חלון של 60 יום ולא של דקות: שינוי של רמאי והתשלום שהוא מכוון אליו רחוקים ימים זה מזה. שמירה חוזרת של אותם פרטים **אינה** שינוי ולא מייצרת רעש. סכימה ×3+migrate. `test/supplier-bank.test.js`. |
