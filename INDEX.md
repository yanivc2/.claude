...
| 6 | **`footer.ejs` — enhancers גלובליים** | `partials/footer.ejs` | בורר-תאריך מחליף כל `input[type=date]` (חשוף כ-`window.apEnhanceDates(root)` לשורות שנוספות דינמית, כמו הוצאות סגירת-Z); קומבובוקס מחליף `<select class="js-combo">`; **מקפל רובריקות** — כל `.card` עם `<h2>` ישיר הופך למתקפל (אקורדיון, נשמר ב-localStorage); **כלי-טבלה** — כל `<table>` עם `<thead>` ו-≥2 שורות מקבל חיפוש חי (מסנן כל העמודות בכל הקשה) + **כפתור "ייצוא" יחיד** (`.tbl-export-toggle`) שפותח תפריט (`.tbl-export-menu`, מוסתר עם `[hidden]`) עם Excel/CSV/PDF של השורות הנראות (עמודות `.no-print`/`[data-noexport]` **מוחרגות** מהייצוא). דילוגים: `.no-collapse`/`.no-enhance`, `<details>`, כרטיס עם `[required]`. שינוי שם משפיע על **כל** הדפים. |
...
|---|---|---|---|---|
| רשימה + סינון (סטטוס/ספק/חנות/תאריך/`q`) + עמודת "שולם" | `GET /invoices` | `listInvoices` → `enrichPaidStatus` (JS, לא JOIN — pg-mem לא תומך ב-join על derived) | טאבים כולל **`unpaid`** וירטואלי. `q` רב-מונחי. עמודת "שולם": אמצעי+מזהה מ-`payment_lines`, "שולם במזומן" אם יש התאמת `z_closing_expenses/z_expenses`, אחרת "לא שולם". חשבונית ששולמה במזומן **ללא** התאמה (`pay_method='cash'` ו-`cash_matches=0`) → שורה אדומה בהירה (`.row-unmatched-cash`). ברירת-מחדל storeId = חנות פעילה. | שינוי מיפוי הסטטוס → משפיע על הטאבים. `enrichPaidStatus` שדות `pay_*`/`cash_matches` נצרכים ב-`views/invoices/index.ejs`. |
...
| תשלום מרוכז | `POST /invoices/pay-batch` | `createPayment` (רב-חשבוניות, זיכוי=שלילי, net=Σ) | `invoice_ids[]` (מפתחות חוזרים!) + אמצעי תשלום → תשלום אחד. נקודת כניסה נוספת: **בחירה מרובה ברשימת החשבוניות** (checkbox `.inv-pick`, אכיפת ספק+חנות אחת ב-JS) → כפתור "הנפק תשלום" מנווט ל-`/invoices/new?supplier=&store=&pick=ids` (אותו מסך batch). | תלוי ב-R1/R5. שינוי encoding (מערך→מחרוזת) שובר. ה-checkbox מסומן רק לחשבוניות payable שלא שולמו. |
...
| הוצאות מזומן (kinds) | אותו POST (`expense_*[]`) | `insertExpenses` → `z_closing_expenses` | ידני/שכר/מפרעה/חשבונית; מקשר עובד/חשבונית. חלק ההזנה בטופס עטוף ב-`<details class="cx-collapse no-collapse">` — **סגור כברירת מחדל** (המשתמש פותח כשצריך; חישוב הסכומים ב-JS רץ גם כשסגור). רובריקה משותפת בדף חשבוניות (`_cashExpenses.ejs`). כל שורה ב-`_cashExpenses` מקשרת ל-`/zclosing/:closing_id` **רק לבעלים** (הדף `requireOwner`); לאחרים טקסט בלבד. | `z_closing_expenses.invoice_id` (nullable, **לא** cascade — ה-clean-start מנתק אותו). הלינק תלוי ב-`closing_id` שמגיע מ-`recentClosingExpenses`. `.cx-collapse` מוגדר ב-`nocturne.css`. |
