...
- **מתי `pay-batch` מפצל:** כשהוזן `pay_amount`, **או** כששאר היתרה כבר קטנה מערך החשבוניות (כלומר צ׳ק קודם כבר כיסה חלק). אחרת נשמר המסלול הישן (`createPayment` עם `invoiceIds`) על כל בדיקותיו. **זיכוי לא נכנס לפיצול** — הוא חייב להיות בתשלום שסוגר את החשבונית (שגיאה מפורשת, לא התנהגות שקטה). `batchContext` מחזיר `open_amount` לכל שורה והטופס מציג "נותרו …" + "יתרה פתוחה במסומנות".
