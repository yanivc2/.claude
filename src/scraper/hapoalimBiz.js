...
export async function scrapeHapoalimBiz({
  ...
  try {
    ...
    if (/\/auth\//.test(page.url())) {
      ...
      // 🔴 להבחין בין "פרטים שגויים" לבין "הבנק מבקש קוד חד-פעמי". נמדד: הפורטל העסקי קיבל
      // את הפרטים ועבר למסך "קוד אימות נשלח לסלולרי שלך" — כלומר ההתחברות **הצליחה** והעצירה
      // היא אימות דו-שלבי. שתי המשמעויות האלה נראות זהות בכתובת ה-URL, ובלי ההבחנה הזו מישהו
      // יבזבז שעות על סיסמה תקינה לחלוטין.
      const otp = await page
        .evaluate(() => /קוד אימות|קוד חד[- ]פעמי/.test(document.body.innerText || ''))
        .catch(() => false);
      if (otp) {
        throw new Error(
          'הבנק דורש קוד אימות חד-פעמי ל-SMS. הפרטים נכונים וההתחברות עברה, אבל סריקה '
          + 'אוטומטית לא יכולה להקליד קוד שנשלח לטלפון — נדרש אדם. ראה INDEX.md · סריקת בנק.',
        );
      }
