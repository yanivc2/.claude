...
/**
 * ההפרש שמוצג כ"חוסר / יתרה", **מנקודת המבט של הקופה**: `(מזומן − הוצאות) − סה"כ הופקד`.
 *
 * 🔴 הכיוון הזה הוא העניין. הפקידו פחות ממה שהיה בקופה → נשארה שם **יתרה** (חיובי). הפקידו יותר
 * ממה שהיה → הקופה ב**חוסר** (שלילי). הסימן נשאר כשהיה (>0 יתרה · <0 חוסר) ולכן כל הקוראים —
 * הרשימה, הוואטסאפ, הטופס — ממשיכים לעבוד בלי שינוי.
 */
export function depositDiff(zr, expenses, depositAmount) {
  return depositBase(zr, expenses) - (Number(depositAmount) || 0);
}
