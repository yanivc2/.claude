...
-- "דוח פדיון" — nightly per-store revenue (sales + credit clearing). Systematic sales source.
...
-- bank_imports — see schema.sql: an upload as one undoable event.
...
-- employee_stores — same shape/rule as supplier_stores (no rows = every store).
...
-- Backfill employee→store links from where employees actually appear, ONCE (only while the table
-- is empty, so it never re-derives over links the owner has edited). Mirrors the SQLite migration.
INSERT INTO employee_stores (employee_id, store_id)
SELECT DISTINCT employee_id, store_id FROM (
  SELECT zc.employee_id AS employee_id, zc.store_id AS store_id FROM z_closings zc
   WHERE zc.employee_id IS NOT NULL AND zc.store_id IS NOT NULL
  UNION
  ...
  UNION
  SELECT ce.employee_id, zc2.store_id FROM z_closing_expenses ce JOIN z_closings zc2 ON zc2.id = ce.closing_id
   WHERE ce.employee_id IS NOT NULL AND zc2.store_id IS NOT NULL
) src
...
-- §5 חשבונות הבנק של הספק — see schema.sql for why this is a table and not columns on suppliers.
...
-- ראה schema.sql: אינדקס אחד עם COALESCE, לא שני אינדקסים חלקיים (pg-mem שובר את החלקיים).
...
ALTER TABLE bank_transactions ADD COLUMN IF NOT EXISTS import_id INTEGER REFERENCES bank_imports(id);
CREATE INDEX IF NOT EXISTS ix_bank_txn_import ON bank_transactions(import_id);
...
-- קוד הבנק נשאר ריק בכוונה: היה רק שם חופשי, והבעלים צריך לאשר מהרשימה מה הבנק בפועל.
...
-- 🏦 תור סנכרון הבנק (הפועלים לעסקים) — ראה schema.sql. בתחתית הקובץ בכוונה: יש כאן מפתח זר
-- ל-users, ומפתח זר מאומת ברגע היצירה (ראה CLAUDE.md · schema in 3 places).
